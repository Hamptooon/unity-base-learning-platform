...
interface ReviewSubmissionFormProps {
  submission: StandalonePracticeSubmission
  reviewerSubmissionId: string
  reviewCriterias: StandalonePractiseCriteria[]
  currentReview: number
  totalReviews: number
  onReviewComplete: (isAllReviewed: boolean) => void
}
  ...
  const handleSubmit = async (values: ReviewFormValues) => {
    ...
    try {
      ...
      const payload = {
        ...
        scores: formattedScores // <- это теперь массив, как нужно
      }

      console.log('Review payload:', payload)

      const isAllReviewed = await practiseService.reviewSubmission(
        submission.id,
        reviewerSubmissionId,
        payload
      )
      console.log('isAllReviewed', isAllReviewed)

      toast.success('Проверка успешно отправлена!')

      onReviewComplete(isAllReviewed)
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="text-xl font-semibold">
        Проверка {currentReview} из {totalReviews}
      </div>
